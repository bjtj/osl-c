...
osl_url_t * osl_url_from_string(const char * url_str)
{
  ...
  if (auth_end)
  {
    ...
    if (auth_sep)
    {
	    url->passphase = osl_safe_strdup(auth_sep + 1);
    }
  ...
  if (address_sep)
  {
    url->port = osl_safe_strdup(address_sep + 1);
  }
  ...
  if (path_start)
  {
    ...
    if (query_start)
    {
	    url->query = osl_safe_strdup(query_start + 1);
    }
  }
...
err:
  ...
  osl_safe_free(auth_part);
  osl_safe_free(address_part);
...
void osl_url_free(osl_url_t * url)
{
  ...
  osl_safe_free(url->protocol);
  osl_safe_free(url->username);
  osl_safe_free(url->passphase);
  osl_safe_free(url->hostname);
  osl_safe_free(url->port);
  osl_safe_free(url->path);
  osl_safe_free(url->query);
  osl_safe_free(url);
}
...
osl_url_t * osl_url_merge(osl_url_t * base_url, const char * relative)
{
  ...
  if (relative[0] == '/')
  {
    url->protocol = osl_safe_strdup(base_url->protocol);
    url->username = osl_safe_strdup(base_url->username);
    url->passphase = osl_safe_strdup(base_url->passphase);
    url->hostname = osl_safe_strdup(base_url->hostname);
    url->query = osl_safe_strdup(base_url->query);
    url->port = osl_safe_strdup(base_url->port);
    url->path = osl_safe_strdup(relative);	
  }
  else
  {
    url->protocol = osl_safe_strdup(base_url->protocol);
